...
def subjWordOrder(config):
    """
    The main function to create the word order for a subject
    Returns:
    wp_tot - the full wordpool, used only for indexing
    subj_wo - subj_wo[x][y][z] gives the zth word on the yth list in session x
    pair_dicts - pair_dicts[x][y] gives the corresponding word for word y when it is
                 presented in session x
    semMat - the matrix of semantic associations
    """
    ...
    semRows, semCols = makeSemBins(semArray)
    ...
    return wp_tot, subj_wo, pair_dicts, semMat


def makeSemBins(semArray):
    """
    ...
    semBins = list(numpy.linspace(semArray.min(), semArray.max(), 5))
...
def choosePairs(semArray, wp_tot, wp_allowed_main, wp_allowed_last, config, semRows, semCols):
    ...
    while session_num < config.nSessions:
        ...
        # Track whether novel words for 24th session have been introduced yet
        already_changed_wp = False
        ...
        while list_num < config.nLists:
            ...
            # If it's the last session, the second half of the lists should contain the words for the last session
            # Only run this once, as subsequent runs will replace unused_sess_wp with the novel words in sess_wp_allowed
            if session_num == config.nSessions - 1 and list_num == config.nLists / 2 and not already_changed_wp:
                unused_sess_wp = sess_wp_allowed[:]
                sess_wp_allowed = wp_allowed_last[:]
                already_changed_wp = True
